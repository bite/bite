...
class MissingInterpolationOption(InterpolationError):
    ...
    def __init__(self, section, option):
        msg = 'section %r: missing option %r in interpolation' % (section, option)
...
class InterpolationEngine(object):
    ...
    def _fetch(self, key):
        ...
        if val is None:
            raise MissingInterpolationOption(self.section.name, key)
...
class ConfigParserExtendedInterpolation(InterpolationEngine):
    """Behaves like ConfigParser ExtendedInterpolation."""
    _cookie = '%'
    ...
    def _fetch(self, key):
        ...
        while True:
            ...
            if val is not None and not isinstance(val, Section):
                break
            if key.startswith('CONFIG:') and getattr(current_section.main, '_config_opts', None):
                val = current_section.main._config_opts.get(key.split(':', 1)[1])
                if val is not None and not isinstance(val, Section):
                    break
            # move up to parent and try again
            # top-level's parent is itself
            if current_section.parent is current_section:
                ...
                if section_name == 'alias' and section_name in current_section.main:
...
interpolation_engines = {
    ...
    'configparser-extended': ConfigParserExtendedInterpolation,
    ...
    def _load(self, infile, configspec):
        if isinstance(infile, str):
            ...
            try:
                ...
                if self.file_error:
                    raise IOError(f'failed reading config file: {infile!r}: {e}')
            else:
